...
- Version: 2.0
- Date: 2026-02
- Purpose: v2 기획 기준 단일 기준 기획서
- Scope: 기획, 시스템 구조, 데이터 구조, 제작/검증 기준(v2 동기화)
...
### 1.1 한 줄 정의
플레이어는 7분 동안 배달 런을 수행하며, 런 중 3번(0:00/3:00/6:00)의 음악 선택으로 규칙을 바꾸고, 수익으로 월드를 확장한다.

### 1.2 장르/경험
- 장르: 캐주얼 액션 + 운영 전략 + 라이트 메타 성장
- 관점: 3D 주행 기반(도시 이동, 상호작용 포인트 중심)
- Viewpoint: Isometric / Quarter-view driving perspective with road-side interaction points.
- 핵심 경험:
  - 짧은 세션 안에서 빠른 판단
...
- 수락 제한: 5초
- 활성 주문 슬롯: 최대 3
- 오퍼 노출: 동시 pending 오퍼 1개 유지, 수락/만료 시 respawn 지연 후 다음 오퍼 생성
- 활성 주문이 3개면 신규 오퍼 스폰을 보류한다
...
- Road는 주행 가능 영역(Drivable)이며, Block은 비주행(Non-drivable) 건물 전용 영역이다.
- Buildings는 Block 위에만 배치한다.
- Player는 Block 영역에 진입할 수 없어야 한다(높이/벽/Collider로 차단).
- Restaurant/Destination은 Block 위 건물로 배치한다.
- 실제 Pickup/Delivery 상호작용은 건물 내부가 아니라, 해당 건물에 가장 가까운 도로 지점(Nearest Road Point)에 스폰된 InteractPoint에서 `F` 입력으로 수행한다.
---
...
- v2 장르 기준:
  - `hiphop`, `ballad`, `edm`, `jazz`, `lofi`, `rock`, `classic`, `disco`
...
- Pickup/Delivery 완료는 트리거 자동 완료가 아니라, Nearest Road Point에 스폰된 InteractPoint에서 `F` 상호작용으로만 완료된다.
- Offer 수락 시 Restaurant 건물의 Nearest Road Point에 Pickup InteractPoint를 스폰하고, Pickup 완료 시 Destination 건물의 Nearest Road Point에 Delivery InteractPoint를 스폰한다.
- InteractPoint는 Success/Fail/Expire/RunEnd/SceneChange 시점에 누수 없이 정리한다.
- Order 흐름은 EventBus(pub/sub) 이벤트 중심으로 결합하고, TTL/타이머는 GameClock(unscaled) 기준으로 처리한다.
